...
class Sessions
{
    ...
    function open()
    {
        $this->dbc = new MYSQLi(SQL_HOST, SQL_USER, SQL_PASSWORD, SQL_BDD)
        OR die('Could not connect to database for Sessions handler.');
    ...
    function read($sid)
    {
        $q = "SELECT `contents` FROM `Sessions` WHERE `id` = '" . $this->dbc->real_escape_string($sid) . "' LIMIT 1";
        $r = $this->dbc->query($q);
    ...
    {
        $q = "REPLACE INTO `Sessions` (`id`, `contents`) VALUES ('" . $this->dbc->real_escape_string($sid) . "', '" . $this->dbc->real_escape_string($data) . "')";
        $this->dbc->query($q);
    ...
    function clean($expire)
    {
        ...
        $q = 'DELETE FROM `Sessions` WHERE `modify_date` < DATE_SUB( NOW(), INTERVAL ' . $sessionMaxLifetime . ' SECOND )';
        $this->dbc->query($q);
